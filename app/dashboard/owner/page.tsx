  ...
  const fetchOwnerData = async () => {
    ...
    try {
      const ownerResponse = await fetch(
        `${API_URLS.GET_WORKER_URL}?pCompanyId=${user.companyId}&pMobileNos=${user.mobile}&pDesignation=Buisnness+Owner&pStaffId=${user.staffId}&pStaffName=&pIsAllStaffLocation=0`
      )
      ...
      if (ownerResponse.ok) {
        ...
        const personsResponse = await fetch(
          `${API_URLS.GET_STAFF_LOCATION_MASTER}?pCompanyId=${user.companyId}`
        )

        if (personsResponse.ok) {
          const personsData = await personsResponse.json()
          setAssignedPersons(personsData || [])
        }
      } else {
        toast.error("Error", {
          ...
          <CardContent className="p-6 flex flex-col items-center">
            {dataLoading ? (
              ...
              <>
                ...
                <p className="text-muted-foreground text-center mb-4">Business Owner</p>
                <p className="text-lg font-medium mb-6">{user.mobile}</p>

                <div className="w-full space-y-2 mb-6">
                  <p className="text-green-600">In Time: {ownerData?.inTime || "-"}</p>
                  <p className="text-red-600">Out Time: {ownerData?.outTime || "-"}</p>
                </div>
